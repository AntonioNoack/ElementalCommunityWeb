...
object GroupsEtc {
    ...
    fun tick() {
        time = java.util.System.nanoTime()
    ...
    init {
        ...
        rainbowColors =
            intArrayOf(0xdd0000, 0xfe6230, 0xfef600, 0x00bb00, 0x009bfe, 0x0808a0, 0x522fa0)
    ...
    fun drawFavourites(
        ...
        if (width > height && allowLeftFavourites) {// on the left
            val fWidth =
                min(height / AllManager.FAVOURITE_COUNT, width * AllManager.MAX_FAVOURITES_RATIO)
            for ((index, favourite) in AllManager.favourites.withIndex()) {
                if (favourite != null) drawElement(
                    canvas, drawCount, drawUUID, 0f, index * fWidth, 0f,
                    fWidth, true, favourite, bgPaint, textPaint
                )
                else drawElement(
                    canvas, -1, 0f, index * fWidth, 0f,
                    fWidth, true, "",
                    -1, -1, bgPaint, textPaint
                )
            }
        } else {// on the bottom
            val fWidth =
                min(width / AllManager.FAVOURITE_COUNT, height * AllManager.MAX_FAVOURITES_RATIO)
            val baseY = height - fWidth
            for ((index, favourite) in AllManager.favourites.withIndex()) {
                if (favourite != null) drawElement(
                    canvas, drawCount, drawUUID, index * fWidth, baseY, 0f,
                    fWidth, true, favourite, bgPaint, textPaint
                )
                else drawElement(
                    canvas, -1, index * fWidth, baseY, 0f,
                    fWidth, true, "", -1, -1, bgPaint, textPaint
                )
            }
        }
    }
    ...
    private fun getCacheEntry(
        ...
        if (entry == null) {
            ...
            val textSize = clamp(
                textPaint.textSize * widthPerNode * 0.8f / width1,
                widthPerNode * 0.02f,
                widthPerNode * (0.8f - spacingFactorX2 * countSize) / max(1, list.size)
            )
    ...
    fun drawElement(
        canvas: Canvas, craftingCount: Int,
        x0: Float, y0: Float, delta: Float,
        widthPerNode: Float, margin: Boolean,
        rawName: String, group: Int, uuid: Int,
        ...
        if (craftingCount > minimumCraftingCount || uuid > -1) {
            ...
            val text = if (craftingCount > minimumCraftingCount)
                if (uuid > -1) "($craftingCount, #$uuid)" else "($craftingCount)"
            else "(#$uuid)"
    ...
    private fun multiplyAlpha(color: Int, multiplier: Float): Int {
        return color.and(0xffffff) or (clamp(color.shr(24).and(255) * multiplier, 0f, 255f).toInt()
            .shl(24))
    }

    fun drawElementBackground(
        canvas: Canvas, x0: Float, y0: Float, delta: Float,
        widthPerNode: Float, margin: Boolean,
        group: Int, bgPaint: Paint
    ) {
        drawElementBackground(
            canvas,
            x0, y0, delta, widthPerNode,
            if (margin) getMargin(widthPerNode) else 0f,
            group, bgPaint
        )
    }
    ...
    fun drawElementBackground(
        ...
        if (group == rainbowIndex) {
            ...
            val alpha1 = bgPaint.alpha.shl(24)
            if (relativeRoundness * rainbowColors.size > 1f) {
                ...
                for (x in x1..x2) {
                    ...
                    val y4 = y2 + bowIndent
                    val xf = x.toFloat()
                    bgPaint.color = rainbowColors[rainbowIndex.toInt()] or alpha1
                ...
                bgPaint.color = rainbowColors[0] or alpha1
                drawRoundRect(canvas, 0f, 0f, perStripe * 2, width, roundness, roundness, bgPaint)

                bgPaint.color = rainbowColors.last() or alpha1
                drawRoundRect(
                    canvas, width - 2 * perStripe, 0f, width, width,
                ...
                for (i in 1 until rainbowColors.size - 1) {
                    ...
                    bgPaint.color = rainbowColors[i] or alpha1
            ...
            drawRoundRect(
                canvas, x0 + d0, y0 + d0, x0 + d1, y0 + d1,
                roundness, roundness, bgPaint
            )
        }
    }
