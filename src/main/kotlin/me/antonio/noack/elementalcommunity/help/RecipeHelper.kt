...
object RecipeHelper {
    ...
    fun readRecipes(raw: String, search: String): ArrayList<Recipe> {
        val recipeStrings = raw.split(';')
        val returnValue = ArrayList<Recipe>()
        for(recipe in recipeStrings) {
            if(recipe.isNotBlank()) {
                val info = recipe.split(':')
                val compA = info.getOrNull(0)?.toIntOrNull() ?: continue
                val compB = info.getOrNull(1)?.toIntOrNull() ?: continue
                val result = info.getOrNull(2)?.toIntOrNull() ?: continue
            ...
            fun showResult(list: List<Recipe>, search: String){
                all.runOnUiThread {
                    ...
                    for(recipe in list){
                        ...
                        recipeView.rName = makeLessReadable(r.name, !isKnown && compacted(search) != compacted(r.name))
                        recipeView.rGroup = r.group
                        recipeView.overlay = if(isKnown) 0 else (0xaa000000.toInt() or all.resources.getColor(R.color.colorPrimaryDark).and(0xffffff))
                        if(!isKnown){
                    ...
                    recipes.visibility = if(recipes.childCount == 0) View.GONE else View.VISIBLE
                }
            }
